// chatbot.js — FINAL STABLE VERSION (Correct Load Order + AI Ready)
(function () {
  /* ================================================================
       LOAD CSS
  ================================================================== */
  const style = document.createElement("link");
  style.rel = "stylesheet";
  style.href = "Chatbot/css/style.css";
  document.head.appendChild(style);

  /* ================================================================
       MAIN CONTAINER
  ================================================================== */
  const main = document.createElement("main");
  main.className = "page-content";

  /* ================================================================
       FLOATING BUTTON
  ================================================================== */
  const chatToggle = document.createElement("button");
  chatToggle.id = "chat-toggle";
  chatToggle.className = "chat-toggle-btn";
  chatToggle.setAttribute("aria-label", "Open ZIA Chatbot");
  // Hidden — dock widget now handles the visible trigger
  chatToggle.style.display = "none";

  chatToggle.innerHTML = `
    <img src="Chatbot/assets/MZHSS.png" alt="Chat" class="chat-icon"/>
    <span class="pulse"></span>
  `;

  /* ================================================================
       CHAT WIDGET
  ================================================================== */
  const chatWidget = document.createElement("section");
  chatWidget.id = "chat-widget";
  chatWidget.className = "chat-widget hidden";
  chatWidget.setAttribute("aria-hidden", "true");

  chatWidget.innerHTML = `
    <header class="chat-header">
      <div class="chat-title">
        <div class="chat-avatar">
          <img src="Chatbot/assets/MZHSS.png" alt="Mount Zion Matric Higher Secondary School" width="40" height="40"/>
        </div>
        <div class="chat-title-text">
          <h2>ZIA</h2>
          <p class="status">Online • Ready to help</p>
        </div>
      </div>

      <button id="chat-close" class="chat-close-btn" aria-label="Close chat">
        <svg width="20" height="20" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" fill="none">
          <path d="M18 6L6 18"></path>
          <path d="M6 6l12 12"></path>
        </svg>
      </button>
    </header>

    <div id="chat-messages" class="chat-messages"></div>

    <form id="chat-form" class="chat-form">
      <div class="input-container">
        <textarea id="chat-input" class="chat-input" rows="1" placeholder="Ask Anything..."></textarea>

        <button type="submit" class="chat-send-btn" aria-label="Send message">
          <svg width="20" height="20" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" fill="none">
            <path d="M22 2L11 13" />
            <path d="M22 2L15 22L11 13L2 9L22 2Z" />
          </svg>
        </button>
      </div>

      <p class="chat-hint">Try: "admission" or "how to reach Mount Zion Matric Higher Secondary School"</p>
    </form>
  `;

  /* ================================================================
       SOUNDS
  ================================================================== */
  const soundTyping = document.createElement("audio");
  soundTyping.id = "typing-sound";
  soundTyping.src = "Chatbot/assets/typing.mp3";
  soundTyping.preload = "auto";

  const soundDelivered = document.createElement("audio");
  soundDelivered.id = "delivered-sound";
  soundDelivered.src = "Chatbot/assets/delivered.mp3";
  soundDelivered.preload = "auto";

  const soundOpen = document.createElement("audio");
  soundOpen.id = "open-sound";
  soundOpen.src = "Chatbot/assets/open.mp3";
  soundOpen.preload = "auto";

  const soundClose = document.createElement("audio");
  soundClose.id = "close-sound";
  soundClose.src = "Chatbot/assets/close.mp3";
  soundClose.preload = "auto";

  /* ================================================================
       APPEND DOM ELEMENTS
  ================================================================== */
  main.appendChild(chatToggle);
  main.appendChild(chatWidget);

  document.body.appendChild(main);
  document.body.appendChild(soundTyping);
  document.body.appendChild(soundDelivered);
  document.body.appendChild(soundOpen);
  document.body.appendChild(soundClose);

  /* ================================================================
       SCRIPT LOADER (with Safe Logging)
  ================================================================== */
  function loadScript(src) {
    return new Promise((resolve) => {
      const s = document.createElement("script");
      s.src = src;
      s.onload = () => {
        console.log(`[Chatbot Loader] Loaded: ${src}`);
        resolve();
      };
      s.onerror = () => {
        console.error(`[Chatbot Loader] FAILED: ${src}`);
        resolve(); // Continue loading others even if one fails
      };
      document.body.appendChild(s);
    });
  }

  /* ================================================================
       LOAD FILES IN PERFECT ORDER
  ================================================================== */
  (async function loadAllScripts() {
    console.log("%c[Chatbot Loader] Starting…", "color:#0099ff; font-weight:bold;");

    await loadScript("Chatbot/js/config.js");    // ChatbotConfig
    await loadScript("Chatbot/js/sound.js");     // Sound controls
    await loadScript("Chatbot/js/ui.js");        // Chat UI system
    await loadScript("Chatbot/js/vista.js");     // Vista + loads panoramas/projects
    await loadScript("Chatbot/js/chat.js");      // AI + backend + intent engine

    console.log("%c[Chatbot Loader] All scripts loaded successfully!", "color:#00C853; font-weight:bold;");
  })();
})();
